...
// menu — a pointer sits on a numbered option (covers AskUserQuestion menus with
// arbitrary labels, not just Yes/Allow). Under ConPTY the pointer is drawn as
// ASCII '>', not the '❯' the TUI uses elsewhere, so accept both. The menu's
// footer hint is the most reliable signal of all. parseApprovalPrompt still
// validates that >=2 options are actually present.
const APPROVAL_RE = /(Do you want to proceed|Do you want to make this edit|Would you like to proceed|Do you trust the files|Enter to select|[❯>]\s*\d+\.\s+\S)/i;
...
class SessionManager {
  ...
  parseApprovalPrompt(session) {
    ...
    // Question: the last recent line ending in '?' (the cap skips the trust
    // prompt's long blurb, which the client renders as a generic heading
    // instead). AskUserQuestion prompts routinely run past 90 chars.
    const qs = lines.filter(l => /\?$/.test(l) && l.length <= 160);
